...
| 릴리스 재현성 | 정정됨[^1] | `v5.0.2` annotated tag가 하드닝을 포함한 릴리스 커밋을 고정함 |
...
[^1]: 원래 이 절은 `v5.0.1` 태그가 릴리스 커밋 `1168193`을 고정한다고 "좋음"으로 평가했다. 2026-09-04 재검토에서 `1168193`은 이 보고서가 기술하는 P0~P2 수정(하드닝 커밋 `2c50137`) 이전 상태이며, `v5.0.1` 태그는 그 하드닝 커밋보다 하루 먼저 생성되어 수정 내용을 전혀 포함하지 않는다는 사실이 확인됐다. `tools/validate_repo.py`를 `git archive v5.0.1` 결과에 직접 실행하면 `UNSUPPORTED`, `TEMP_CLEANUP`, `TEMP_RETAIN`, `RESUME` 등 10개 finding이 나온다 — 이 보고서가 "완료"로 표시한 항목들이 태그된 릴리스에는 반영되지 않았다는 뜻이다. `PATCHES-K.md`의 버전 규칙("K-only fix는 patch 버전을 올린다")에 따라 하드닝 커밋을 `5.0.2`로 재버전하고 그 커밋에 새 태그를 만들어 바로잡았다. `v5.0.1` 태그 자체는 이미 존재하는 참조이므로 이동하지 않고 보존했다.
